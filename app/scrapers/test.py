...
class Scraper(BaseScraper):
    ...
    def scrape(self) -> list[dict]:
        ...
        if ul is None:
            raise Exception("Diagnostic: Could not locate chapter `<ul>`. (Tried matching 'epcheck', 'bxcl', id='chapterlist', class='eplister', and `<li data-num>`)")

        episodes = []
        for li in ul.find_all("li", recursive=False):
            parsed = self._parse_li(li)
            if parsed:
                episodes.append(parsed)

        if not episodes:
            raise Exception("Diagnostic: Found chapter `<ul>`, but failed to parse any episodes from its `<li>` elements.")
